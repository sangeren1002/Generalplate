...
void uart_init(u32 bound){
   //GPIO端口设置
	...
	GPIO_Init(GPIOA,&GPIO_InitStructure); //初始化PA9与PA10
   ...
   //USART1 初始化配置
	...
	NVIC_Init(&NVIC_InitStructure);	//根据参数初始化NVIC寄存器
...
{
	u8 Res;
#if SYSTEM_SUPPORT_UCOS  //只用UCOS系统
	OSIntEnter();    
#endif
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)  //接收中断(接收到数据必须是0x0d 0x0a结尾)
	{
