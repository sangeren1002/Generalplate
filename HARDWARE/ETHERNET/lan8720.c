...
u8 LAN8720_Init(void)
{
	...
	GPIO_PinAFConfig(GPIOA, GPIO_PinSource1, GPIO_AF_ETH); //引脚复用到网络接口
	...
	GPIO_PinAFConfig(GPIOC, GPIO_PinSource1, GPIO_AF_ETH); //引脚复用到网络接口
...
#endif
	//当我们使用帧校验和卸载功能时，一定要使能存储转发模式，存储转发模式中要保证整个帧存储在FIFO中
	//这样MAC能插入/识别出帧校验值，当真校验值正确的时候DMA就可以处理帧，否则就丢弃掉该帧
...
extern void lwip_pkt_handle(void);		//在lwip_comm.c里面呢定义
//以太网DMA接收中断服务函数
void ETH_IRQHandler(void)
{
	...
	ETH_DMAClearITPendingBit(ETH_DMA_IT_R); 	//清楚DMA中断标志位
	ETH_DMAClearITPendingBit(ETH_DMA_IT_NIS);	//清楚DMA接收中断标志位
}  
//接收一个网卡数据包
//返回值：网络数据包帧结构体
...
{ 
	...
	//检查当前描述符是否属于ETHERNET DMA(设置的时候)/CPU(复位的时候)
	if((DMARxDescToGet->Status&ETH_DMARxDesc_OWN)!=(u32)RESET)
	{	
		frame.length=ETH_ERROR; 
		if ((ETH->DMASR&ETH_DMASR_RBUS)!=(u32)RESET)  
		{ 
			ETH->DMASR = ETH_DMASR_RBUS;//清除ETH DMA的RBUSλ 
...
{   
	...
	DMATxDescToSet->Status|=ETH_DMATxDesc_LS|ETH_DMATxDesc_FS;//设置最后一个和第一个为段置位(1个描述符传输一帧)
	...
	{ 
		ETH->DMASR=ETH_DMASR_TBUS;//重置ETH DMA TBUSλ 
		ETH->DMATPDR=0;//恢复DMA发送
	} 
	//更新ETH DMA全局Tx描述符为下一个Tx描述符
	//为下一次的buffer发送设置一个DMA Tx描述符
